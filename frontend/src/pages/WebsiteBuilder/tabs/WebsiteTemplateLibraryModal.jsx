...
import { Globe, X as CloseIcon, Search as SearchIcon, CheckCircle, Upload as UploadIcon, Trash2, Eye } from "lucide-react";
import WebsiteTemplatePreviewModal from "./WebsiteTemplatePreviewModal";
...
const WebsiteTemplateLibraryModal = ({ open, onCancel, onCreate, initialWebsiteName, initialShowUploadBox = false }) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.data && data.data._id) {
          ...
          if (!websiteName.trim()) {
            setWebsiteName(`${data.data.name} Website`);
          }
  ...
  const handleSelectTemplate = (template) => {
    ...
    if (!websiteName.trim()) {
      setWebsiteName(`${template.name} Website`);
    }
  };
  ...
  return (
    <>
                  ...
                  <Text type="secondary">
                    {isCreating ? (
                      <span style={{ color: "var(--accent-info)", fontWeight: 500, display: "flex", alignItems: "center", gap: 8 }}>
                        <Spin size="small" /> Website template is currently being prepared...
                      </span>
                    ) : (
                      "Select or preview a template to jump-start your website project"
                    )}
                  </Text>
                </div>
                <Space>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 24 }}>
              <Row gutter={[24, 24]}>
                {filteredTemplates.map(template => {
                  ...
                  const isHovered = hoveredTemplateId === template._id;
                  return (
                    <Col span={12} key={template._id}>
                      <Card 
                        ...
                        bodyStyle={{ padding: 0 }}
                      >
                        <div style={{ height: 160, background: template.thumbnailColor || "linear-gradient(135deg, #0f172a, #1e293b)", position: 'relative', overflow: 'hidden', padding: 20 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", position: 'relative', zIndex: 2 }}>
                            ...
                            <Tag style={{ margin: 0, background: "rgba(255, 255, 255, 0.2)", border: "none", color: "#fff", fontWeight: 700, borderRadius: 6 }}>{template.featuresCount || 1} Pages</Tag>
                          </div>
                          ...
                          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.6), rgba(0,0,0,0.2))', zIndex: 1 }} />
                          ...
                          >
                            ...
                            <Button
                              type="primary"
                              icon={isSelected ? <CheckCircle size={16} /> : undefined}
                              onClick={(e) => {
                                e.stopPropagation();
                                handleSelectTemplate(template);
                              }}
                              style={{
                                background: isSelected ? "var(--accent-success)" : "var(--accent-info)",
                                border: "none",
                                color: "#fff",
                                borderRadius: 8,
                                fontWeight: 700,
                        ...
                        <div style={{ padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            {isSelected && (
                              <div style={{ color: "var(--accent-info)", fontSize: 12, fontWeight: 800, display: "flex", alignItems: "center", marginLeft: 4 }}>
                                <CheckCircle size={16} style={{ marginRight: 4 }} /> SELECTED
                              </div>
                            )}
                          </div>
                        </div>
      ...
      {/* Full UI Live Interactive Preview Modal */}
      <WebsiteTemplatePreviewModal
        ...
        isSelected={selectedTemplate === previewingTemplate?._id}
      />
    </>
  );
};
