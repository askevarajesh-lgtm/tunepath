...
const UserDashboard = () => {
  ...
  // Metrics for Top Cards (All Time or active)
  ...
  const tasksForSelectedDate = useMemo(() => {
    return tasks.filter(t => {
      if (!t.dueDate) return false;
      return dayjs(t.dueDate).isSame(selectedDate, 'day');
    });
  }, [tasks, selectedDate]);

  // Performance Score Card Metrics (Current Month)
  const currentMonthTasks = useMemo(() => {
    const startOfMonth = dayjs().startOf('month');
    const endOfMonth = dayjs().endOf('month');
    return tasks.filter(t => {
      const dateToCheck = dayjs(t.createdAt);
      return dateToCheck.isAfter(startOfMonth) && dateToCheck.isBefore(endOfMonth);
    });
  }, [tasks]);

  const perfMetrics = useMemo(() => {
    const assigned = currentMonthTasks.length;
    const done = currentMonthTasks.filter(t => isCompleted(t.status)).length;
    const inProg = currentMonthTasks.filter(t => isInProgress(t.status)).length;
    const pending = currentMonthTasks.filter(t => isPending(t.status)).length;
    const correction = currentMonthTasks.filter(t => isCorrectionTask(t)).length;
    const redesign = currentMonthTasks.filter(t => isRedesignTask(t)).length;
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        ...
        <motion.div variants={itemVariants} style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Button type="text" icon={<ChevronLeft size={16} />} onClick={() => setSelectedDate(prev => prev.subtract(1, 'day'))} />
            <DatePicker 
              value={selectedDate} 
              ...
              style={{ width: 140 }}
            />
            <Button type="text" icon={<ChevronRight size={16} />} onClick={() => setSelectedDate(prev => prev.add(1, 'day'))} />
          </div>
          {isUserRole && (
            <Button 
              type="primary" 
