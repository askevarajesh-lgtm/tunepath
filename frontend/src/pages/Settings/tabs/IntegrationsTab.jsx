  ...
  if (selectedConfig) {
    return (
      <div style={{ marginTop: 16 }}>
        ...
        {selectedConfig.type === 'ivr' && <IvrConfigPage integrationId={selectedConfig.id} onBack={handleBack} />}
      </div>
    );
  }
    ...
    <>
      ...
      <div>
        ...
        <div className="int-grid">
          ...
          {canSeeIntegration('ivr') && (
            <IntegrationCard
              type="ivr"
              integration={ivrIntegration}
              icon={<IvrIcon />}
              title="Sollu IVR / Telephony"
              description="Configure your Sollu cloud telephony credentials, DID, and webhook receiver for outbound calling"
            />
          )}
        </div>
        {!isPlatformAdmin && ['whatsapp', 'sms', 'email', 'website', 'payment', 'ekta', 'ivr'].filter(canSeeIntegration).length === 0 && (
