...
// Public Custom Domain Resolution Endpoint
exports.resolveWebsiteByDomain = async (req, res, next) => {
    try {
        let rawDomain = req.query.domain;
        if (!rawDomain) {
            rawDomain = req.headers['x-forwarded-host'] || req.headers.host || req.hostname;
        }
        if (!rawDomain) {
            return res.status(400).json({ success: false, error: 'Domain is required' });
        }

        // Clean & normalize domain: trim, lowercase, strip port number if present
        let domainStr = rawDomain.toString().trim().toLowerCase();
        domainStr = domainStr.split(':')[0]; // strip port

        // Reserved platform domains list
        const reservedPlatformDomains = [
            'localhost',
            '127.0.0.1',
            'tunepath.askeva.io',
        ...
        if (process.env.PLATFORM_DOMAIN) {
            reservedPlatformDomains.push(process.env.PLATFORM_DOMAIN.toLowerCase());
        }

        if (reservedPlatformDomains.some(plat => domainStr === plat || domainStr.endsWith('.' + plat))) {
            return res.status(200).json({
                success: true,
                isPlatformDomain: true,
                message: 'Platform domain requested'
            });
        }

        // Build domain lookup variants (www. vs non-www, with or without ports)
        const cleanVariants = [domainStr];
        if (domainStr.startsWith('www.')) {
            cleanVariants.push(domainStr.replace(/^www\./, ''));
        } else {
            cleanVariants.push(`www.${domainStr}`);
        }

        const domainRegexes = cleanVariants.map(v => new RegExp('^' + v.replace(/\./g, '\\.') + '(:\\d+)?$', 'i'));
        ...
        let website = null;
        if (domainDoc) {
            website = await Website.findOne({ _id: domainDoc.propertyId, isDeleted: false });
        }

        if (!website) {
            return res.status(404).json({
                success: false,
                error: 'Website not found for domain',
                domain: domainStr
            });
        }
        ...
        reqPath = reqPath.toString().trim();
        if (!reqPath.startsWith('/')) {
            reqPath = '/' + reqPath;
        }

        // Find pages for website
        const allPages = await Page.find({ websiteId: website._id, isDeleted: false }).sort({ createdAt: 1 });
        if (!allPages || allPages.length === 0) {
            return res.status(404).json({
                success: false,
                error: 'No pages found for website',
        ...
        if (reqPath === '/' || reqPath === '') {
            matchedPage = allPages.find(p => p.isHome) || allPages.find(p => p.path === '/' || p.path === '') || allPages[0];
        } else {
            const cleanReqPath = reqPath.toLowerCase();
            matchedPage = allPages.find(p => {
                const pagePath = (p.path || '').toLowerCase();
                return pagePath === cleanReqPath || ('/' + pagePath.replace(/^\//, '')) === cleanReqPath;
            });
        }
        ...
        if (!matchedPage) {
            return res.status(404).json({
                ...
                website: website.toObject(),
                pagesSummary: allPages.map(p => ({ _id: p._id, title: p.title, path: p.path, isHome: p.isHome }))
            });
        }

        // Optionally fetch widget
        let chatWidget = null;
        if (website.chatWidgetId) {
            try {
                const Widget = require('../widgets/widget.model');
                chatWidget = await Widget.findOne({ _id: website.chatWidgetId, isDeleted: false });
            } catch (e) {
                // ignore widget error
            }
        }
