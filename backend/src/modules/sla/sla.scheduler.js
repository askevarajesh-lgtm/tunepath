...
const runSlaCheck = async () => {
  ...
  try {
    ...
    for (const task of pendingTasks) {
      ...
      // A task is ONLY Breached if current time is past 11:59:59 PM on its due date
      if (now > dueDate) {
        status = 'Breached';
      }

      // DO NOT trigger or display SLA record if task is still within its due date (Normal)
      if (status === 'Normal') {
        await SlaRecord.deleteOne({ entityId: task._id, entityType: 'Task' });
        continue;
      }
      ...
      // Dispatch notification if status escalated to Breached
      if (updatedSla && status === 'Breached' && oldStatus !== 'Breached') {
        await dispatchSystemNotification(
          task.tenantCompanyId,
          'sla_breached',
          'SLA Trigger',
          `SLA Breached: ${updatedSla.title}`,
          `Task ${task.title} has not been completed by the 11:59 PM due date deadline.`,
...
// Run every hour & immediately on startup
const startSlaScheduler = () => {
  runSlaCheck().catch(err => console.error("Initial SLA Check error:", err));
